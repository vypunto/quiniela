...
export default function WeekView({ publications, onSelect, activeFilter }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow border border-gray-200 overflow-hidden flex flex-col" style={{ minHeight: '340px' }}>
      {/* Week nav */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 bg-gray-50 flex-shrink-0">
        <button onClick={prevWeek} className="w-8 h-8 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-colors">
        ...
        <button onClick={nextWeek} className="w-8 h-8 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-colors">
      ...
      <div className="grid grid-cols-7 divide-x divide-gray-100 flex-1">
        {days.map(day => {
          ...
          return (
            <div key={day.toISOString()} className={`flex flex-col min-h-[200px] ${isToday ? 'bg-rose-50/60' : ''}`}>
              {/* Day header */}
              <div className="px-1 pt-2 pb-1.5 text-center border-b border-gray-100 flex-shrink-0">
                <div className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest">
                ...
                <div className={`text-sm sm:text-base font-black mx-auto w-7 h-7 flex items-center justify-center rounded-full mt-0.5 ${
                  isToday ? 'bg-[#732442] text-white' : 'text-gray-700'
                }`}>
                  {day.getDate()}
                </div>
              </div>
