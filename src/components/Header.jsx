      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 h-[58px] flex items-center gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 mr-1 sm:mr-3 flex-shrink-0">
          ...
          <div className="leading-none">
            <div className="text-[11px] sm:text-sm font-black tracking-tight" style={{ color: '#732442' }}>CALENDAPP</div>
            <div className="text-[8px] sm:text-[10px] font-medium mt-0.5 text-gray-400">by El Chandrio Group</div>
          </div>
        </div>
        ...
        {activeTab === 'publicaciones' && (
          <div className="hidden sm:block">
            {isDemo ? (
              ...
              <button onClick={onDemo} className="px-2.5 py-1 rounded-lg border border-gray-200 text-gray-400 text-xs font-medium hover:bg-gray-50 transition-colors">
                Ver demo
              </button>
            )}
          </div>
        ...
        {hasConfig && activeTab === 'publicaciones' && (
          <button onClick={onSync} disabled={loading} title="Sincronizar" className="hidden sm:flex w-9 h-9 rounded-lg border border-gray-200 items-center justify-center hover:bg-gray-50 transition-colors text-gray-500 disabled:opacity-50">
        ...
        {/* Settings */}
        <button onClick={onSettings} title="Configuración" className="w-9 h-9 rounded-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors text-gray-500 flex-shrink-0">
      ...
      {activeTab === 'publicaciones' && (
        <div className="sm:hidden flex items-center justify-between px-3 pb-2 gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Demo toggle mobile */}
            {isDemo ? (
              <button onClick={onExitDemo} className="px-2 py-1 rounded-lg text-[10px] font-semibold active:opacity-80" style={{ border: '1px solid #732442', color: '#732442', backgroundColor: '#fdf0f3' }}>
                Salir demo
              </button>
            ) : (
              <button onClick={onDemo} className="px-2 py-1 rounded-lg border border-gray-200 text-gray-400 text-[10px] font-medium active:bg-gray-100">
                Demo
              </button>
            )}
            ...
            {/* Sync */}
            {hasConfig && (
              <button onClick={onSync} disabled={loading} className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 disabled:opacity-50 active:bg-gray-100">
